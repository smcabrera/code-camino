...
describe "User" do
  ...
  context "Signed in" do
    ...
    it "should be able to create a new lesson for one of her paths" do
      path = create(:path, :user => @alice)
      visit path_path(path)
      click_link "Add Lesson"

      fill_in "lesson[name]", with: "New Lesson"
      fill_in "lesson[url]", with: "www.example.com/lesson"
      click_button "Save"
    ...
    it "should be able to edit lessons on paths she created" do
      path = create(:path, :user => @alice)
      lesson = create(:lesson, :user => @alice, :path => path )
      ...
      fill_in "lesson[name]", with: "Edited"
      click_button "Save"

      expect(Lesson.first.name).to eq("Edited")
    end

    it "should NOT be able to edit lessons on paths created by someone else" do
    ...
    it "should NOT be able to delete edit lessons on paths created by someone else" do
      bobs_path = create(:path, :user => @bob)
